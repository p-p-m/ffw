...
class CartProduct(object):
    ...
    # TODO: rewrite this class to use product table configuration from cart
    # settings
    ...
    def __init__(self, product_pk):
        ...
        try:
            ...
            raise CartException(
                "Can not find product with pk {}".format(product_pk))
...
class Cart(object):
    ...
    def _calculate(self):
        ...
        self.cart['total'] = float(
            round(sum([v['sum_'] for v in self.cart['products'].values()]), 2))
        self.cart['count'] = sum([v['quant']
                                  for v in self.cart['products'].values()])
    ...
    def remove(self, product_pk):
        ...
        try:
            ...
            raise CartException(
                'Cart does not contain product with key {}'.format(
                    product.pk_str))
        self._calculate()
