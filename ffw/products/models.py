...
class ImageFieldWaterMark(models.ImageField):
    def pre_save(self, model_instance, add):
        ...
        self.angle = 23
        self.opacity = 0.25
        self.mark_text = "TTTTTTTTTT"
        self.font = 'Arial.ttf'
        self.__add_watermark(file.path, self.mark_text, self.angle, self.opacity, )
        return file

    def __add_watermark(self, file_path, mark_text, angle, opacity):
        img = Image.open(file_path).convert('RGB')
        watermark = Image.new('RGBA', img.size, (0,0,0,0))
        size = 2
        FONT = self.font
        n_font = ImageFont.truetype(FONT, size)
        n_width, n_height = n_font.getsize(mark_text)
